...
`SEARCH-05` 已在上述表达层之后增加受控执行链：

- FTS 和结构化条件只在当前本地 `SearchBoundary` 中召回，AI 不直接读取数据库；
- 默认最多向重排请求提供 20 条候选，并同时受单题字符数和总载荷字节数约束；
- 默认 JSONL 只含 ID、标题、题干、知识路径、标签和更新时间；答案、个人作答、错因、备注与原图不在默认范围；
- 重排响应执行独立固定 Schema 校验，随后再次以本轮候选 ID 集合校验，虚构、重复和超限 ID 会被拒绝；
- 网络或格式失败不会绕过本地边界，也不会回退为整库上传。

`SEARCH-06` 已将这条链路接入题库界面：

- AI 搜索在后台线程执行，显示意图解析、本地召回和候选重排阶段；
- 完成后展示候选量、发送量、披露字段、阶段耗时、token、估算费用和重试次数；
