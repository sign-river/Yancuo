...
4. `assets/index.json`
5. `checksums.sha256`（完整覆盖上述四项、可选 identity 及 index 声明的全部对象）

损坏向量：任意改动 `snapshot.sqlite` 字节而不更新 checksums、空校验表、遗漏固定载荷、对象索引与实际文件不一致、manifest 与快照 schema/题目计数不一致，恢复均必须失败。
