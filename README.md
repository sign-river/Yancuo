...
- 笔记基础模型：schema v8 新增独立的 NoteDocument / NoteBlock / NoteAsset 与笔记标签关系；笔记不复用题目字段，AI 提取与统一搜索仍待接入
- 笔记本地编辑：侧栏新增笔记页，可按生命周期浏览、创建笔记、编辑标题摘要及文本/标题/公式/提示块，并在同页切换阅读预览；原图交互和统一搜索仍在后续切片
- AI 笔记录入：选择图片并补充提示词后，后台调用已配置视觉 Provider，生成可编辑笔记草稿；确认页预览原图和内容块，确认后才写入笔记与原图资产
...
- 题目 AI 已收到现有分类路径并被要求只选已有路径；后续将升级为稳定分类 ID、近义去重和人工确认的新分类建议，笔记分类也将复用同一规则。
