...
一次性上传地址只允许一个正在执行的 PUT：网关先在 PostgreSQL 中原子认领会话，失败时释放认领以供安全重试。对象已经完整落盘但客户端未收到响应时，同一凭据重试会幂等返回成功，不会覆盖已上传对象；完成状态必须同时具有文件 ID、正确大小和唯一会话更新。过期对象删除失败时保留会话记录，后续清理继续重试，不能先丢失对象索引。
